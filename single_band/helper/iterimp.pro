function iterimp, orig, specdens, sigm_gauss, binwidth, SIGM=sigm, NUM_PULSES=num_pulses, no_opt=no_opt
compile_opt idl2, HIDDEN
    ...
    ;   sigm_y0     - theoretical deviations of ys
...
if keyword_set(num_pulses) then begin
    for i = 1, num_pulses do begin
        ...
        if keyword_set(SIGM) then begin
            ...
            sigm_y0 = [sigm_y0, retval.sigm_y0]
...
if keyword_set(no_opt) then begin
    ...
    if (n_elements(As) gt 0) then optval = estopt(signal, specdens, As, xs, ys, sigm_gauss, binwidth) else return, {As:[-1], signal:orig}
