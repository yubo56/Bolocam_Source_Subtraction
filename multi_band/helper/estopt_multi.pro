function estopt_multi, signal, specdens, As, xs, ys, sigm_gauss, binwidth
...
cnvrgnceFact    = 10 ; considers a peak's position determined when iteration changes position by less than sigma_x/cnvrgnceFact
numConsec       = 5 ; number of consecutive subtractions that must change by less than sigm_x/cnvrgnceFact
maxcnv          = 100 ; maximum number of iterations before giving up
...
while TOTAL(flags) gt 0 do begin ; while at least one flag needs to be examined
    for i=0,(N_ELEMENTS(As) - 1) do begin
        ...
        if flags[i] ne 0 then begin
            ; perform subtraction
            signal = addgauss_multi(As[i], sigm_gauss, xs[i], ys[i], signal) ; add signal back
            retVal = subtractmax_multi(signal, specdens, sigm_gauss, binwidth)
            signal = retval.sig

            ; flag appropriately
            if (abs(retVal.xparam - xs[i]) le retval.sigm_x0 / cnvrgnceFact) and (abs(retVal.yparam - ys[i]) le retval.sigm_x0 / cnvrgnceFact) then begin
